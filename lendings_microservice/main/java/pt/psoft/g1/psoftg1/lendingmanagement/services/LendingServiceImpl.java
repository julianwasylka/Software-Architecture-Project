...
public class LendingServiceImpl implements LendingService{
    ...
    public Lending setReturned(final String lendingNumber, final SetLendingReturnedRequest resource) {
        ...
        lendingEventsPublisher.publishLendingReturnedEvent(lending, resource);

        return lending;
    }

    @Transactional
    public void rollbackLending(String lendingNumber) {
        var lending = lendingRepository.findByLendingNumber(lendingNumber)
                .orElseThrow(() -> new NotFoundException("Cannot rollback lending with this lending number"));

        lending.setCommentary(null);
        lending.setReturnedDate(null);
        lendingRepository.save(lending);

        var fine = fineRepository.findByLendingNumber(lendingNumber);
        fine.ifPresent(fineRepository::delete);
    }
